...
<div class="row g-3 g-md-4">
    ...
    <div class="col-12 col-lg-4">
        ...
        <!-- PM Surya Ghar National Portal Unique ID Card -->
        <div class="card border-0 shadow-sm rounded-3 border-start border-4 border-success mb-4">
            <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center">
                <h6 class="m-0 font-outfit fw-bold text-navy">
                    <i class="bi bi-patch-check-fill text-success me-2"></i> PM Surya Ghar National ID
                </h6>
                <button type="button" class="btn btn-outline-success btn-sm fw-bold" data-bs-toggle="modal" data-bs-target="#modalUpdatePmId">
                    <i class="bi bi-pencil-square me-1"></i> <?= empty($customer['pm_surya_ghar_id']) ? 'Set ID' : 'Edit' ?>
                </button>
            </div>
            <div class="card-body">
                <?php if (!empty($customer['pm_surya_ghar_id'])): ?>
                    <div class="mb-2">
                        <span class="text-muted small d-block">PM Surya Ghar Consumer / App ID:</span>
                        <span class="badge bg-success text-white font-monospace fs-6 px-3 py-2 mt-1">
                            <?= htmlspecialchars($customer['pm_surya_ghar_id']) ?>
                        </span>
                    </div>
                    <?php if (!empty($customer['notification_number'])): ?>
                    <div>
                        <span class="text-muted small d-block">Notification Reference No:</span>
                        <span class="font-monospace fw-bold text-navy small"><?= htmlspecialchars($customer['notification_number']) ?></span>
                    </div>
                    <?php endif; ?>
                <?php else: ?>
                    <div class="text-center py-2">
                        <span class="badge bg-warning text-dark px-3 py-2 mb-2 d-inline-block">
                            <i class="bi bi-exclamation-circle me-1"></i> ID Not Updated Yet
                        </span>
                        <p class="small text-muted mb-2">Once customer registration on PM Surya Ghar National Portal is complete, update the generated Consumer ID here.</p>
                        <button type="button" class="btn btn-success btn-sm fw-bold w-100" data-bs-toggle="modal" data-bs-target="#modalUpdatePmId">
                            <i class="bi bi-link-45deg me-1"></i> Link PM Surya Ghar ID
                        </button>
                    </div>
                <?php endif; ?>
            </div>
        ...
        <!-- Modal: Update PM Surya Ghar ID -->
        <div class="modal fade" id="modalUpdatePmId" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form action="<?= url('/boe/customers/' . $customer['id'] . '/update-pm-id') ?>" method="POST">
                        <?= csrf_field() ?>
                        <div class="modal-header bg-navy text-white">
                            <h6 class="modal-title font-outfit fw-bold">
                                <i class="bi bi-patch-check-fill text-warning me-2"></i> Update PM Surya Ghar Portal ID
                            </h6>
                            <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                        </div>
                        <div class="modal-body">
                            <p class="small text-muted mb-3">
                                Enter the unique Consumer / Application ID generated by the <strong>PM Surya Ghar National Portal</strong>. This ID will immediately be visible to the customer on their dashboard and profile.
                            </p>
                            <div class="mb-3">
                                <label class="form-label small fw-bold text-navy">PM Surya Ghar Unique Consumer / Application ID <span class="text-danger">*</span></label>
                                <input type="text" name="pm_surya_ghar_id" class="form-control font-monospace" placeholder="e.g. PMSGY-OD-2026-981245" value="<?= htmlspecialchars($customer['pm_surya_ghar_id'] ?? '') ?>" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label small fw-bold text-navy">Scheme Notification Number (Optional)</label>
                                <input type="text" name="notification_number" class="form-control" placeholder="e.g. NOTIF/2026/DSI/042" value="<?= htmlspecialchars($customer['notification_number'] ?? '') ?>">
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Cancel</button>
                            <button type="submit" class="btn btn-success btn-sm fw-bold px-3">
                                <i class="bi bi-check2-circle me-1"></i> Save & Sync ID
                            </button>
                        </div>
                    </form>
                </div>
    ...
    <div class="col-12 col-lg-8">
        ...
        <!-- PM SURYA GHAR AUTO-GENERATED DOCUMENTS UPLOAD SECTION -->
        <div class="card border-0 shadow-sm rounded-3 mb-4 border-start border-4 border-primary">
            <div class="card-header bg-white py-3 d-flex justify-content-between align-items-center flex-wrap gap-2">
                <div>
                    <h6 class="m-0 font-outfit fw-bold text-navy">
                        <i class="bi bi-cloud-arrow-up-fill text-primary me-2"></i> PM Surya Ghar Generated Documents (Feasibility, Bank Sanction & Congratulation)
                    </h6>
                    <span class="text-muted small">Upload scheme auto-generated reports from PM Surya Ghar Portal for customer visibility</span>
                </div>
                <button type="button" class="btn btn-primary btn-sm fw-bold shadow-sm" data-bs-toggle="modal" data-bs-target="#modalUploadPmDoc">
                    <i class="bi bi-upload me-1"></i> Upload PM Document
                </button>
            </div>
            <div class="card-body">
                <div class="row g-2">
                    <?php
                    $pmDocTypes = [
                        'FEASIBILITY_REPORT' => ['title' => 'DISCOM Feasibility Report', 'icon' => 'bi-file-earmark-check', 'color' => 'success'],
                        'BANK_CONGRATULATION_LETTER' => ['title' => 'Bank Sanction / Congrats Letter', 'icon' => 'bi-bank', 'color' => 'primary'],
                        'PM_SURYA_GHAR_CONGRATS' => ['title' => 'PM Surya Ghar Congrats Page', 'icon' => 'bi-patch-check', 'color' => 'warning'],
                        'APPLICATION_ACKNOWLEDGEMENT' => ['title' => 'Acknowledgement of Application', 'icon' => 'bi-receipt', 'color' => 'info'],
                    ];
                    foreach ($pmDocTypes as $pType => $pMeta):
                        $foundDoc = null;
                        foreach ($documents as $d) {
                            if (($d['document_type'] ?? '') === $pType) {
                                $foundDoc = $d;
                                break;
                            }
                        }
                        ...
                        <div class="p-3 bg-light rounded-3 border h-100 d-flex flex-column justify-content-between">
                            <div>
                                <div class="d-flex align-items-center gap-2 mb-1">
                                    <i class="bi <?= $pMeta['icon'] ?> text-<?= $pMeta['color'] ?> fs-5"></i>
                                    <span class="fw-bold small text-navy text-truncate"><?= $pMeta['title'] ?></span>
                                </div>
                                <?php if ($foundDoc): ?>
                                    ...
                                    <div class="small text-muted text-truncate" style="font-size: 0.75rem;">
                                        <?= htmlspecialchars(date('d M Y', strtotime($foundDoc['created_at']))) ?>
                                    </div>
                                <?php else: ?>
                                    <span class="badge bg-secondary-subtle text-secondary border mb-2">
                                        <i class="bi bi-clock me-1"></i> Pending Upload
                                    </span>
                                <?php endif; ?>
                            </div>
                            <div class="mt-2">
                                <?php if ($foundDoc): ?>
                                    <a href="<?= url('/document/download?file=' . urlencode($foundDoc['file_path'])) ?>" target="_blank" class="btn btn-sm btn-outline-primary w-100 py-1" style="font-size: 0.75rem;">
                                        <i class="bi bi-eye me-1"></i> View / Download
                                    </a>
                                <?php else: ?>
                                    <button type="button" class="btn btn-sm btn-outline-success w-100 py-1" style="font-size: 0.75rem;" onclick="openPmDocModal('<?= $pType ?>')">
        ...
        <!-- Modal: Upload PM Surya Ghar Document -->
        <div class="modal fade" id="modalUploadPmDoc" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <form action="<?= url('/boe/customers/' . $customer['id'] . '/upload-pm-doc') ?>" method="POST" enctype="multipart/form-data">
                        <?= csrf_field() ?>
                        <div class="modal-header bg-navy text-white">
                            <h6 class="modal-title font-outfit fw-bold">
                                <i class="bi bi-cloud-arrow-up-fill text-warning me-2"></i> Upload PM Surya Ghar Scheme Document
                            </h6>
                            <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                        </div>
                        <div class="modal-body">
                            <div class="mb-3">
                                ...
                                <select name="document_type" id="selectPmDocType" class="form-select" required>
                                    <option value="FEASIBILITY_REPORT">DISCOM Technical Feasibility Report</option>
                                    <option value="BANK_CONGRATULATION_LETTER">Bank Sanction & Congratulation Letter</option>
                                    <option value="PM_SURYA_GHAR_CONGRATS">PM Surya Ghar Congratulation Page</option>
                                    <option value="APPLICATION_ACKNOWLEDGEMENT">Acknowledgement of Application</option>
                                </select>
                            </div>
                            <div class="mb-3">
                                <label class="form-label small fw-bold text-navy">Select File / Scan (PDF, JPG, PNG) <span class="text-danger">*</span></label>
                                <input type="file" name="document_file" class="form-control" accept="image/*,.pdf" capture="environment" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label small fw-bold text-navy">Remarks / Notes (Optional)</label>
                                <input type="text" name="remarks" class="form-control" placeholder="e.g. Loan sanctioned by SBI / Feasibility approved by TPCODL">
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Cancel</button>
                            <button type="submit" class="btn btn-primary btn-sm fw-bold px-3">
                                <i class="bi bi-upload me-1"></i> Upload to Customer Account
                            </button>
                        </div>
                    </form>
                </div>
        ...
        <script>
        function openPmDocModal(docType) {
            const sel = document.getElementById('selectPmDocType');
            if (sel) sel.value = docType;
            const modal = new bootstrap.Modal(document.getElementById('modalUploadPmDoc'));
            modal.show();
        }
        </script>
